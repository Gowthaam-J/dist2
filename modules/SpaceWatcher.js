...
class SpaceWatcher extends events_1.default {
    ...
    async downloadAudio() {
        ...
        try {
            ...
            // Import named functions from StorageManager
            const { uploadFileToS3, saveFileLinkToDB } = await Promise.resolve().then(() => __importStar(require('./StorageManager')));
            ...
            try {
                ...
                const s3AudioUrl = await uploadFileToS3(filePath, s3AudioKey);
                const s3PlaylistUrl = await uploadFileToS3(playlistPath, s3PlaylistKey);
                const s3JsonlUrl = await uploadFileToS3(tmpFile, s3JsonlKey);
                const s3TxtUrl = await uploadFileToS3(outFile, s3TxtKey);
                ...
                await saveFileLinkToDB({ space_id: this.spaceId, s3_url: folderUrl });
    ...
    async downloadCaptions() {
        ...
        try {
            ...
            // After captions extraction, upload files to S3 and save metadata to DB
            const { uploadFileToS3, saveFileLinkToDB } = await Promise.resolve().then(() => __importStar(require('./StorageManager')));
            const s3AudioKey = `spaces/${this.spaceId}/${this.filename}.m4a`;
            const s3PlaylistKey = `spaces/${this.spaceId}/${this.filename}.m3u8`;
            const s3JsonlKey = `spaces/${this.spaceId}/${this.filename} CC.jsonl`;
            const s3TxtKey = `spaces/${this.spaceId}/${this.filename} CC.txt`;
            const filePath = path_1.default.join(Util_1.Util.getMediaDir(username), `${this.filename}.m4a`);
            const playlistPath = path_1.default.join(Util_1.Util.getMediaDir(username), `${this.filename}.m3u8`);
            const tmpFilePath = path_1.default.join(Util_1.Util.getMediaDir(username), `${this.filename} CC.jsonl`);
            const outFilePath = path_1.default.join(Util_1.Util.getMediaDir(username), `${this.filename} CC.txt`);
            try {
                const s3AudioUrl = await uploadFileToS3(filePath, s3AudioKey);
                const s3PlaylistUrl = await uploadFileToS3(playlistPath, s3PlaylistKey);
                const s3JsonlUrl = await uploadFileToS3(tmpFilePath, s3JsonlKey);
                const s3TxtUrl = await uploadFileToS3(outFilePath, s3TxtKey);
                // Construct folder URL for spaceId
                const bucketName = process.env.AWS_S3_BUCKET_NAME || '';
                const region = process.env.AWS_REGION || '';
                const folderUrl = `https://${bucketName}.s3.${region}.amazonaws.com/spaces/${this.spaceId}/`;
                ...
                this.logger.error(`Failed to upload captions or save metadata: ${error}`);
            }
        }
        catch (error) {
